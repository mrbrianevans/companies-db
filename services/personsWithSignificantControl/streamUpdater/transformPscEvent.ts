...
export function addPscIdAndCompanyNumber<EventType extends PscEvent>(event: EventType): EventType['data'] & {company_number: string, psc_id: string}{
  const psc_ids = event.data.links.self.match(/\/company\/([A-Z\d]{8})\/persons-with-significant-control\/(corporate-entity|individual|legal-person|individual-beneficial-owner)\/(.+)/)
